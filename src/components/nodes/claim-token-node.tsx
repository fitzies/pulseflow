...
export const ClaimTokenNode = memo(({ data }: NodeProps) => {
  const nodeData = data as ClaimTokenNodeData;
    ...
    <div className={NODE_WRAPPER_CLASS}>
      <NodeCardWithStatus status={status} className={`${NODE_WIDTH_CLASS} ${NODE_HEIGHT_CLASS} cursor-pointer`} onClick={handleNodeClick}>
        ...
        <div className="flex items-center justify-center p-3">
          <div className={`rounded-xl ${getNodeBackgroundColor('claimToken')} p-3 flex items-center justify-center`}>
      ...
      {nodeData?.showNodeLabels !== false && (
        <div className={NODE_LABEL_CONTAINER_CLASS}>
          <span className={NODE_TITLE_CLASS}>Claim Token</span>
...
ClaimTokenNode.displayName = 'ClaimTokenNode';
