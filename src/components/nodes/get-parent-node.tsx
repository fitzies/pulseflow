...
export const GetParentNode = memo(({ data }: NodeProps) => {
  const nodeData = data as GetParentNodeData;
    ...
    <div className={NODE_WRAPPER_CLASS}>
      <BaseNode className={`${NODE_WIDTH_CLASS} ${NODE_HEIGHT_CLASS} cursor-pointer`} onClick={handleNodeClick}>
        ...
        <div className="flex items-center justify-center p-3">
          <div className={`rounded-xl ${getNodeBackgroundColor('getParent')} p-3 flex items-center justify-center`}>
            <BuildingOfficeIcon className={`h-8 w-8 ${getNodeTextColor('getParent')}`} />
          </div>
        </div>
      ...
      {nodeData?.showNodeLabels !== false && (
        <div className={NODE_LABEL_CONTAINER_CLASS}>
          <span className={NODE_TITLE_CLASS}>Get Parent</span>
...
GetParentNode.displayName = 'GetParentNode';
