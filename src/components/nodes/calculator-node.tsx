...
import { CalculatorIcon, PlusIcon } from '@heroicons/react/24/solid';
...
export const CalculatorNode = memo(({ data }: NodeProps) => {
  const nodeData = data as CalculatorNodeData;
    ...
    <div className={NODE_WRAPPER_CLASS}>
      <NodeCardWithStatus status={status} className={`${NODE_WIDTH_CLASS} ${NODE_HEIGHT_CLASS} cursor-pointer`} onClick={handleNodeClick}>
        ...
        <div className="flex items-center justify-center p-3">
          <div className={`rounded-xl ${getNodeBackgroundColor('calculator')} p-3 flex items-center justify-center`}>
            <CalculatorIcon className={`h-8 w-8 ${getNodeTextColor('calculator')}`} />
          </div>
        </div>
      ...
      {nodeData?.showNodeLabels !== false && (
        <div className={NODE_LABEL_CONTAINER_CLASS}>
          <span className={NODE_TITLE_CLASS}>Calculator</span>
