...
export const CheckPriceNode = memo(({ data }: NodeProps) => {
  const nodeData = data as CheckPriceNodeData;
    ...
    <div className={NODE_WRAPPER_CLASS}>
      <NodeCardWithStatus status={status} className={`${NODE_WIDTH_CLASS} ${NODE_HEIGHT_CLASS} cursor-pointer`} onClick={handleNodeClick}>
        ...
        <div className="flex items-center justify-center p-3">
          <div className={`rounded-xl ${getNodeBackgroundColor('checkPrice')} p-3 flex items-center justify-center`}>
      ...
      {nodeData?.showNodeLabels !== false && (
        <div className={NODE_LABEL_CONTAINER_CLASS}>
          <span className={NODE_TITLE_CLASS}>Check Price</span>
...
CheckPriceNode.displayName = 'CheckPriceNode';
