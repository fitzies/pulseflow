...
import { getNodeBackgroundColor, getNodeTextColor, NODE_WRAPPER_CLASS, NODE_WIDTH_CLASS, NODE_HEIGHT_CLASS, NODE_LABEL_CONTAINER_WIDE_CLASS, NODE_TITLE_CLASS, NODE_NOTES_CLASS } from './node-colors';

interface CheckTokenBalanceNodeData {
  onAddNode?: () => void;
  onNodeClick?: () => void;
  isLastNode?: boolean;
...
export const CheckTokenBalanceNode = memo(({ data }: NodeProps) => {
  const nodeData = data as CheckTokenBalanceNodeData;
    ...
    <div className={NODE_WRAPPER_CLASS}>
      <NodeCardWithStatus status={status} className={`${NODE_WIDTH_CLASS} ${NODE_HEIGHT_CLASS} cursor-pointer`} onClick={handleNodeClick}>
        ...
        <div className="flex items-center justify-center p-3">
          <div className={`rounded-xl ${getNodeBackgroundColor('checkTokenBalance')} p-3 flex items-center justify-center`}>
      ...
      {nodeData?.showNodeLabels !== false && (
        <div className={NODE_LABEL_CONTAINER_WIDE_CLASS}>
          <span className={NODE_TITLE_CLASS}>Check Token Balance</span>
...
CheckTokenBalanceNode.displayName = 'CheckTokenBalanceNode';
